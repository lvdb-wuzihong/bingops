...
| 12 | ~~builder 新增桥接/派生规则~~ **✅ 已完成** | 已落地：node→云主机承载于（#27）、NAT DNAT 派生边（kind=dnat）、EIP 直绑 kind=bind、PV→云盘 CSI 桥接（#43/#44/#56，kind=csi，附录 B #32）、ingress 路由上游（#30）、**service→CLB/NLB LB 桥接**（#38/#39，kind=lb，附录 B #33）。仅余 dns→apisix #48 随 apisix 缓做 |
