...
async def create_ticket(session: AsyncSession, payload: TicketCreate, operator: User) -> Ticket:
    ...
    if payload.business_app_id is not None:
        app = await session.get(CmdbBusinessApp, payload.business_app_id)
        if app is None:
            raise NotFoundError("CmdbBusinessApp", str(payload.business_app_id))
...
# ── 工单解决飞书私聊提醒（应用机器人单聊，复用 SSO 自建应用凭据）──────────
...
def _build_resolved_card(ticket: Ticket, operator: User, comment: str | None) -> dict:
    ...
    return {
        ...
        "header": {
            "title": {"tag": "plain_text", "content": "工单已解决"},
            "template": "green",
        },
        "elements": elements,
    }
